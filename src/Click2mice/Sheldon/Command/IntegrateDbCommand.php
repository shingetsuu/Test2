...
class IntegrateDbCommand extends ProcessCommand
{
    protected $dbName;
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp(
                ...
                'Название базы данных (например: wm2)'
            )->addArgument(
                'version-name',
                InputArgument::REQUIRED,
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->dbName = $input->getArgument('db-name');
        ...
        $output->writeln("Имя БД: <comment>$this->dbName</comment>");
        $component = 'db-wm2';
        ...
        if ($issues['total']) {
            ...
            $versionDir = $repo->get_repo_path() . "/{$this->dbName}/" . $numericVersion;
            ...
            foreach ($issues['issues'] as $issue) {
                ...
                if (empty($allowIntegrateError)) {
                    ...
                    try {
                        ...
                        $issuePath = $repo->get_repo_path() . "/{$this->dbName}/" . $issueDir;
                        if (!is_dir($issuePath)) {
                            $output->writeln('<error>директория тикета ' . $issueDir . ' не существует</error>');
                            ...
                            // move dir in git
                            $output->writeln($issueDir . " -> {$this->dbName}/" . $numericVersion);
                            if (!$dryRun) {
            ...
            if (!empty($mergedIssueKeys)) {
                if (!$dryRun) {
                    // commit
                    $output->writeln('Коммитим ' . $versionName);

                    $message = 'Adding ' . implode(', ', $mergedIssueKeys) . " to {$this->dbName}/" . $numericVersion;
                    $repo->commit($message);
    ...
    {
        $project = $this->getProject('db-wm2');
